...
st.markdown('#### Enter the stock ticker')
  ...
  # RESHAPING THE DATA
    ...
    # GRAPH AFTER THE MODEL
        def plot_pred_data(stock_name):
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=train_part['Date'],
                                     y=train_part['Close'],mode='lines',name='Train'))
            fig.add_trace(go.Scatter(x=valid['Date'],
                                     y=valid['Close'],mode='lines',name='Actual'))
            fig.add_trace(go.Scatter(x=valid['Date'],
                                     y=valid['Prediction'],mode='lines',name='Prediction'))
        ...
        def get_price():
            ...
            answer = st.markdown(f"#### The closing price on {pred_date} will approximately be {predictions_1[0][0]}")
